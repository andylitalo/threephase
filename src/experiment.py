...
This document defines the Experiment class along with the subclass PressureStep
...
# GLOBAL CONSTANTS
P_ATM = 14.7 # atmospheric pressure (parr) [psi]
...
class Experiment:
    ...
    def load_p_step(self, p_step):
        """Loads PressureStep object p_step to list p_step_list in Experiment object."""
        # Only load PressureStep object if not already present
        if p_step not in self.p_step_list:
            self.p_step_list += [p_step]

    def get_p(self):
        """Returns pressures of each experiment in chronological order."""
        return np.array([m.p for p_step in self.p_step_list for m in p_step.measurement_list])

    def get_T(self):
        """Returns pressures of each experiment in chronological order."""
        return np.array([m.T for p_step in self.p_step_list for m in p_step.measurement_list])

    def get_meas(self, p_min=0, p_max=1500, T_min=-247, T_max=60):
        """Returns measurements within limits of p [psi] and T [C] given."""
        return [m for p_step in self.p_step_list for m in p_step.measurement_list
    ...
    def get_m_co2_leaked(self, leak_rate_list):
        ...
        for p_step in self.p_step_list:
            ...
            for m in p_step.measurement_list:
                # get time since start of pressure step [min]
                dt = TimeDate.diff_min(p_step.timedate, m.timedate)
    ...
    def to_df(self, m_co2_leak_list=[]):
        ...
        df = pd.DataFrame(columns=['date', 'time', 'elapsed time [min]',
                        'pressure (parr) [psi]', 'temperature (parr) [C]',
                        'mass co2 [g]', 'mass co2 (leak) [g]', 'mass co2 [g] (corr)',
                        'mass c5 [g]', 'mass poly [g]',
                        ...
                        'density poly (liq) [g/mL]', 'density co2 (tot) [g/mL]',
                        'density co2 (tot) [g/mL] (corr)',
                        'density c5 (tot) [g/mL]', 'density poly (tot) [g/mL]',
                        'w co2 (vap) [w/w]', 'w c5 (vap) [w/w]', 'w co2 (liq) [w/w]',
                        'w c5 (liq) [w/w]', 'w poly (liq) [w/w]',
                        'w co2 (tot) [w/w]', 'w c5 (tot) [w/w]',
                        'w poly (tot) [w/w]', 'w co2 (tot) [w/w] (corr)', 'w c5 (tot) [w/w] (corr)',
                        'w poly (tot) [w/w] (corr)', 'liquid volume [mL]',
                        'mass c5 (pred) [g]', 'mass c5 (missing) [g]',
        ...
        for i in range(n):
            ...
            m_tot = m.m_co2 + m.m_c5 + m.m_poly + m.p_step.experiment.m_impurities
            ...
            df['mass poly [g]'].iloc[i] = m.m_poly
            ...
            df['pressure (parr) [psi]'].iloc[i] = m.p
            df['temperature (parr) [C]'].iloc[i] = m.T
            ...
            df['density poly (liq) [g/mL]'].iloc[i] = m.rho_l_poly
            df['density co2 (tot) [g/mL]'].iloc[i] = m.m_co2 / m.p_step.experiment.V ###
            df['density c5 (tot) [g/mL]'].iloc[i] = m.m_c5 / m.p_step.experiment.V
            df['density poly (tot) [g/mL]'].iloc[i] = m.m_poly / m.p_step.experiment.V
            ...
            df['w poly (liq) [w/w]'].iloc[i] = m.w_l_poly
            df['w co2 (tot) [w/w]'].iloc[i] = m.m_co2/(m_tot) ###
            df['w c5 (tot) [w/w]'].iloc[i] = m.m_c5/(m_tot)
            df['w poly (tot) [w/w]'].iloc[i] = m.m_poly/(m_tot)
            ...
            if len(m_co2_leak_list) > 0:
                ...
                m_tot_corr = m_co2_corr + m.m_c5 + m.m_poly + m.p_step.experiment.m_impurities
                df['mass co2 [g] (corr)'].iloc[i] = m_co2_corr
                df['mass co2 (leak) [g]'].iloc[i] = m_co2_leak
                df['density co2 (tot) [g/mL] (corr)'].iloc[i] = m_co2_corr / m.p_step.experiment.V ###
                df['w co2 (tot) [w/w] (corr)'].iloc[i] = m_co2_corr/(m_tot_corr) ###
                df['w c5 (tot) [w/w] (corr)'].iloc[i] = m.m_c5/(m_tot_corr)
                df['w poly (tot) [w/w] (corr)'].iloc[i] = m.m_poly/(m_tot_corr)
    ...
    def save_metadata(self, file_hdr='metadata'):
        ...
        ***LIKELY NON-OPERATIVE BECAUSE PYTHON WILL NOT PICKLE EXPERIMENT OBJECTS.
        ...
        metadata = {'volume [mL]':self.V, 'mass impurities [g]':self.m_impurities,
                    ...
                    'offset of Parr pressure (parr) [psi]':self.p_parr_offset}
        with open(file_hdr + '_{0:d}'.format(100*self.m_c5_0) + '%c5.pkl', 'wb') as f:
            pkl.dump(metadata, f)

class PressureStep(Experiment):
    """
    A collection of "measurements" (objects of the Measurement class) taken after
    a single pressurization of the Parr reactor with the ISCO pump (or change in
    pressure through some other mechanism, such as changing temperature).
    """
    def __init__(self, experiment, p0, T0, p_isco_i, V_isco_i, p_isco_f, V_isco_f,
                timedate, p_isco_refill=-1, T_isco=-274, c5=False):
        """
        Creates PressureStep object initialized with the given parameters.
        PARAMETERS:
            self : PressureStep object
        ...
        experiment.load_p_step(self)
        # initialize list of measurements included in this pressure step
        self.measurement_list = []
        # record time and date of pressurization
        self.timedate = timedate

        # separate assignment in case this is the only pressure step in the experiment
        self.p_isco_offset = 0
        p_isco_offset = experiment.p_step_list[-1].p_isco_offset
        # if no new temperature given for ISCO, assign to value set for experiment
        if T_isco == -274:
            self.T_isco = experiment.T_isco
        ...
        if p_isco_refill > 0:
            ...
            m_co2_i = tern.rho_co2(p_isco_i-self.p_isco_offset, self.T_isco, psi=True)*V_isco_i
            m_co2_full = tern.rho_co2(experiment.p_tank, self.T_isco, psi=True)*experiment.V_isco_full
        ...
        elif p_isco_refill < -1:
            # use refill pressure to determine offset (should match tank pressure)
            self.p_isco_offset = (-p_isco_refill) - experiment.p_tank
            # compute how much co2 was in ISCO before refilling
            m_co2_f = tern.rho_co2(p_isco_f-self.p_isco_offset, self.T_isco, psi=True)*V_isco_f
        ...
        self.p_isco_i = p_isco_i-self.p_isco_offset
        self.V_isco_i = V_isco_i
        self.p_isco_f = p_isco_f-self.p_isco_offset
        self.V_isco_f = V_isco_f

        # compute masses of co2 before and after pressurization  [g]
        if not c5:
            m_co2_i = tern.rho_co2(self.p_isco_i, self.T_isco, psi=True)*V_isco_i
            m_co2_f = tern.rho_co2(self.p_isco_f, self.T_isco, psi=True)*V_isco_f
            self.m_co2_isco = m_co2_f
            # add co2 lost from isco to dispensed co2 in Parr
            experiment.m_co2 += m_co2_i - m_co2_f
        else:
            m_c5_i = tern.rho_c5(self.T_isco)*V_isco_i
            m_c5_f = tern.rho_c5(self.T_isco)*V_isco_f
            self.m_co2_isco = 0
...
class Measurement(PressureStep):
    """
    ...
    def pred_m_c5(self):
        ...
        self.m_c5_pred = self.V_liq*self.rho_l_c5 + (self.p_step.experiment.V-self.V_liq)*self.rho_v_c5

    def pred_m_co2(self):
        """Computes missing mass of co2 as sign of 3-phase region"""
        # estimate cyclopentane mass by adding up masses in liquid and vapor phases
        self.m_co2_pred = self.V_liq*self.rho_l_co2 + (self.p_step.experiment.V-self.V_liq)*self.rho_v_co2
    ...
    def __init__(self, p_step, pa_f_co2, pa_f_c5, pa_b_co2, pa_b_c5, p_parr, T,
        ...
        PARAMETERS:
            ...
            mixing_stopped : bool, default=True
                Purely an indication whether mixing by the built-in mixing shaft
                was paused during sampling.
            sampled_liquid : bool, default=True
                If True, indicates that liquid phase was sampled through the HPLIS
                and that the estimated mass of the liquid phase components should be
                incremented down based on the approximate volume sampled (an
                Experiment parameter V_per_meas) and the weight fractions of the
                components in the liquid phase.
            vented_headspace : bool, default=False
                If
        ...
        self.diffusion_time = TimeDate.diff_min(p_step.timedate, timedate)
        ...
        p_step.load_measurement(self)
        # store ISCO data
        if p_isco == -1:
            p_isco = experiment.p_isco - self.p_step.p_isco_offset
        if T_isco == -274:
            T_isco = experiment.T_isco
        self.p_isco = p_isco - self.p_step.p_isco_offset
        self.T_isco = T_isco
        self.V_isco = V_isco
